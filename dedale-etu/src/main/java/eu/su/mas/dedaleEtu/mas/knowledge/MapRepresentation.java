...
public class MapRepresentation implements Serializable,Cloneable {
	...
	public static SerializableSimpleGraph<String,MapAttribute> difference(SerializableSimpleGraph<String,MapAttribute> newMap ,SerializableSimpleGraph<String,MapAttribute> oldMap) {
		
		SerializableSimpleGraph<String,MapAttribute> result =new SerializableSimpleGraph<String,MapAttribute>();
		Iterator<SerializableNode<String, MapAttribute>> iter=newMap.getAllNodes().iterator();
		while(iter.hasNext()){
			SerializableNode<String, MapAttribute> n=iter.next();
			
		    if (!( oldMap.getAllNodes().contains(newMap.getNode(n.getNodeId() )) ))
		    	
			result.addNode(n.getNodeId(),MapAttribute.valueOf((String)n.getNodeContent().name()));
		}
